 ...
 * <p>
 * Each render tick, when no packet-driven animation is active on a player, the highest-priority
...
public final class ConditionalAnimations {
    ...
    public record ConditionalAnimation(Predicate<Player> condition, String key, int priority, boolean firstPerson) {}

    private static final List<ConditionalAnimation> REGISTRY = new ArrayList<>();

    public static void register(Predicate<Player> condition, String key, int priority, boolean firstPerson) {
    ...
    public static void register(Predicate<Player> condition, String key) {
        register(condition, key, 0, false);
    }

    /** Highest-priority conditional whose predicate matches the player, or {@code null}. */
    public static ConditionalAnimation evaluate(Player player) {
        for (ConditionalAnimation conditional : REGISTRY) {
            if (conditional.condition().test(player)) return conditional;
        }
        return null;
    }
}
